...
import { PromptTemplateActionsMenu } from './prompt-template-actions-menu';
import type { PromptTemplate } from '../utils/use-prompt-templates';
        ...
        >
          ...
          <div className='min-w-0'>
            ...
            <CardDescription className='truncate text-sm'>
              {promptTemplate.description || 'No description'}
            </CardDescription>
          </div>
        </Link>

        {canManage && (
          <PromptTemplateActionsMenu
            promptTemplate={promptTemplate}
      ...
      <CardContent className='flex items-center justify-between'>
        <div className='flex items-center gap-2'>
          ...
          <span className='text-muted-foreground text-xs'>
            v{promptTemplate.mainVersion?.version ?? promptTemplate.version}
          </span>
        </div>
        <span className='text-muted-foreground text-xs'>
          Updated {new Date(promptTemplate.updatedAt).toLocaleDateString()}
        </span>
      </CardContent>
    </Card>
  );
