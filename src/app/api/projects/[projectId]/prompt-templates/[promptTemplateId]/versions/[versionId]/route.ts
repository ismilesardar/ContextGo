/**
 * DELETE /api/projects/[projectId]/prompt-templates/[promptTemplateId]/versions/[versionId]
 * Deletes a single version snapshot (org owner/moderator only).
 * Guarded: a PromptTemplate must always keep at least one version, and the
...
const deleteHandler: ApiHandler = async (_req, { apiContext, params }) => {
  ...
  if (targetVersion.id === promptTemplate.mainVersionId) {
  ...
  if (versionCount <= 1) {
    return NextResponse.json(
      {
        error: 'A promptTemplate must always have at least one version.',
  ...
  await recordProjectActivity({
    ...
    action: 'prompt_template_version.deleted',
