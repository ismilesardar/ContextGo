...
import { AgentProfileCardSkeleton } from './agent-profile-card-skeleton';

type StatusFilter = 'all' | 'draft' | 'published';
type SortOption = 'updated' | 'title';

export function AgentProfilesListView({
  workspaceSlug,
  projectId
}: {
  ...
  return (
    <PageShell
      ...
      description='Bundle Instructions, Skills, Prompt Templates, and Checklists into a reusable AI role package — e.g. Backend Developer, QA, Marketing.'
      showDate={false}
      actions={
        canManage && (
          <Button asChild>
            <Link
              href={`/${workspaceSlug}/projects/${projectId}/agent-profiles/new`}
            >
      ...
      {isError ? (
        ...
        <div className='grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3'>
          {Array.from({ length: 6 }).map((_, i) => (
            <AgentProfileCardSkeleton key={i} />
          ))}
        </div>
      ) : (sortedAgentProfiles?.length ?? 0) === 0 ? (
        <div className='flex flex-col items-center justify-center rounded-2xl border border-dashed border-neutral-300 p-16 text-center dark:border-neutral-700'>
          ...
          <p className='text-muted-foreground text-sm'>
            {hasActiveFilter
              ? 'No agent profiles match your search.'
              : 'No Agent Profiles yet. Create your first one to bundle resources into a reusable AI role.'}
          </p>
        ...
        <div className='grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3'>
          {sortedAgentProfiles?.map((agentProfile: AgentProfile) => (
            <AgentProfileCard
              key={agentProfile.id}
