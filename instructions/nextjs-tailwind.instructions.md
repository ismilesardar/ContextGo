...
This template uses a fixed set of libraries. Always reach for these instead of introducing alternatives.

| Area | Library / Location |
|---|---|
| Authentication | `better-auth` — catch-all at `/api/auth/[...all]`, config in `src/lib/auth/auth.ts`, client in `src/lib/auth/auth-client.ts` |
| Session access | `src/lib/auth/auth-session.ts` — use this for server-side session checks. There is **no `middleware.ts`**. |
| Server state / data fetching | `@tanstack/react-query` |
| Client state | `zustand` — stores live in `src/store/` |
| URL search params | `nuqs` |
| Data tables | `@tanstack/react-table` |
| Forms & validation | `react-hook-form` + `zod` + `@hookform/resolvers` |
| UI components | shadcn/ui (Radix UI base) — components in `src/components/ui/` |
| Rate limiting | `@arcjet/next` |
| Error tracking | `@sentry/nextjs` — configured in `src/instrumentation.ts` |
| Billing | `stripe` + `creem` — both wired as Better Auth plugins in `src/lib/auth/auth.ts` |
| Email | `nodemailer` via `src/lib/send-mail.ts` |
| File storage | `@aws-sdk/client-s3` (Backblaze B2 endpoint) — utilities in `src/lib/storage/` |
