...
export function InstructionsListView({
  workspaceSlug,
  projectId
}: {
  ...
  return (
    <PageShell
      title='Instructions'
      description='AI behavior rules, coding conventions, naming standards, security requirements, and team workflows.'
      showDate={false}
      actions={
        canManage && (
          <Button asChild>
    ...
    >
      ...
      {isError ? (
        ...
        <div className='flex flex-col items-center justify-center rounded-2xl border border-dashed border-neutral-300 p-16 text-center dark:border-neutral-700'>
          ...
          <p className='text-muted-foreground text-sm'>
            {hasActiveFilter
              ...
              : 'No Instructions yet. Create your first Instruction to document important project knowledge.'}
          </p>
        ...
        <div className='grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3'>
          {sortedInstructions?.map((instruction: Instruction) => (
            <InstructionCard
              key={instruction.id}
