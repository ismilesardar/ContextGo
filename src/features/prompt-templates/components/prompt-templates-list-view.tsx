...
export function PromptTemplatesListView({
  ...
  return (
    <PageShell
      ...
      description='A reusable prompt library for common AI tasks — copy-ready prompts your team can drop straight into any AI tool.'
      showDate={false}
      actions={
        canManage && (
    ...
    >
      ...
      {isError ? (
      ...
      ) : (sortedPromptTemplates?.length ?? 0) === 0 ? (
        <div className='flex flex-col items-center justify-center rounded-2xl border border-dashed border-neutral-300 p-16 text-center dark:border-neutral-700'>
          ...
          <p className='text-muted-foreground text-sm'>
            {hasActiveFilter
              ...
              : 'No PromptTemplates yet. Create your first PromptTemplate to document important project knowledge.'}
          </p>
        ...
        <div className='grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3'>
          {sortedPromptTemplates?.map((promptTemplate: PromptTemplate) => (
            <PromptTemplateCard
